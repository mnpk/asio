...
class json {
  ...
  */
  template <typename T, typename std::enable_if<
                            std::numeric_limits<T>::is_integer, T>::type = 0>
  json(const T n) noexcept : type_(value_t::number),
  ...
  */
  ...
  json(const T n) noexcept : type_(value_t::number_float),
  ...
  template <
      ...
      typename std::enable_if<
          std::is_constructible<std::string, typename V::key_type>::value and
              std::is_constructible<json, typename V::mapped_type>::value,
          int>::type = 0>
  json(const V& v)
      : json(object_t(v.begin(), v.end())) {}
 ...
 private:
  ...
  std::string dump(const bool, const unsigned int, unsigned int = 0) const
      noexcept;
 ...
 public:
  ...
  template <class T, typename std::enable_if<
                         std::is_constructible<json, T>::value>::type = 0>
 ...
 public:
  ...
  /// a const iterator
  class const_iterator
      : public std::iterator<std::bidirectional_iterator_tag, const json> {
    friend class json;
...
inline void swap(nlohmann::json& j1, nlohmann::json& j2) noexcept(
    is_nothrow_move_constructible<nlohmann::json>::
        value and is_nothrow_move_assignable<nlohmann::json>::value) {
  j1.swap(j2);
}
...
json::value::value(number_float_t _number_float)
    : number_float(_number_float) {}
...
*/
json::json(const value_t t) : type_(t) {
  switch (type_) {
    case (value_t::array): {
...
json::json(const char* s) : type_(value_t::string), value_(new string_t(s)) {}

json::json(const bool b) noexcept : type_(value_t::boolean), value_(b) {}

json::json(const array_t& a) : type_(value_t::array), value_(new array_t(a)) {}
...
json::json(list_init_t a) {
  ...
  for (const auto& element : a) {
    if (element.type_ != value_t::array or element.size() != 2 or
  ...
  for (const json& element : a) {
    ...
    value_.object->emplace(std::make_pair(std::move(k), std::move(element[1])));
  }
}
...
*/
json::json(const json& o) : type_(o.type_) {
  switch (type_) {
    case (value_t::array): {
...
*/
json::json(json&& o) noexcept : type_(std::move(o.type_)),
...
@return a JSON object
*/
json json::parse(const char* s) { return parser(s).parse(); }
...
json::operator std::string() const { return get<std::string>(); }

json::operator int() const { return get<int>(); }

json::operator int64_t() const { return get<int64_t>(); }

json::operator double() const { return get<double>(); }

json::operator bool() const { return get<bool>(); }

json::operator array_t() const { return get<array_t>(); }

json::operator object_t() const { return get<object_t>(); }
...
std::string json::dump(const bool prettyPrint, const unsigned int indentStep,
  ...
  switch (type_) {
    ...
    case (value_t::array): {
      ...
      for (array_t::const_iterator i = value_.array->begin();
           i != value_.array->end(); ++i) {
    ...
    case (value_t::object): {
      ...
      for (object_t::const_iterator i = value_.object->begin();
           i != value_.object->end(); ++i) {
...
*/
void json::replaceChar(std::string& str, char c,
                       const std::string& replacement) const {
  size_t start_pos = 0;
...
json::reference json::operator[](const char* key) {
  ...
  if (type_ != value_t::object) {
    throw std::domain_error("cannot add entry with key " + std::string(key) +
                            " to " + type_name());
  }
...
json::const_reference json::operator[](const std::string& key) const {
  // this [] operator only works for objects
  if (type_ != value_t::object) {
    throw std::domain_error("cannot get entry with key " + std::string(key) +
                            " from " + type_name());
  }
...
*/
json::reference json::at(const std::string& key) { return at(key.c_str()); }
...
json::value_t json::type() const noexcept { return type_; }

json::iterator json::find(const std::string& key) { return find(key.c_str()); }
...
bool json::operator==(const json& o) const noexcept {
  switch (type_) {
    ...
    case (value_t::number_float): {
      if (o.type_ == value_t::number) {
        return value_.number_float ==
               static_cast<number_float_t>(o.value_.number);
      }
      if (o.type_ == value_t::number_float) {
...
json::iterator json::begin() noexcept { return json::iterator(this, true); }

json::iterator json::end() noexcept { return json::iterator(this, false); }
...
bool json::iterator::operator==(const json::iterator& o) const {
  if (object_ != nullptr and o.object_ != nullptr) {
    if (object_->type_ == json::value_t::array and
        o.object_->type_ == json::value_t::array) {
      return (*vi_ == *(o.vi_));
    }
    if (object_->type_ == json::value_t::object and
        o.object_->type_ == json::value_t::object) {
      return (*oi_ == *(o.oi_));
    }
...
std::string json::iterator::key() const {
  if (object_ == nullptr or invalid or
      object_->type_ != json::value_t::object) {
    throw std::out_of_range("cannot get value");
  }
...
*/
json::parser::parser(const char* s) : buffer_(s) {
...
*/
json::parser::parser(const std::string& s) : buffer_(s) {
  // read first character
  next();
}
...
json json::parser::parse() {
  switch (current_) {
    ...
    case ('9'): {
      ...
      while (next() and
             (std::isdigit(current_) or current_ == '.' or current_ == 'e' or
              current_ == 'E' or current_ == '+' or current_ == '-'))
        ;

      try {
        const auto float_val =
            std::stold(buffer_.substr(_firstpos_, pos_ - _firstpos_));
...
unsigned int json::parser::parse4HexCodePoint() {
  ...
  for (; pos_ < startPos + 4; pos_++) {
    ...
    if ((currentChar >= '0' and currentChar <= '9') or
        (currentChar >= 'a' and currentChar <= 'f') or
        (currentChar >= 'A' and currentChar <= 'F')) {
